...
- Bônus "grupo perfeito": +100 — só quando o palpite reproduz a classificação real em **todas** as posições (inclusive nulos). Grupo com apenas 2 classificados (3º real nulo) exige que o usuário também tenha deixado o 3º em branco; palpitar um 3º que não classificou quebra o bônus. Implementado de forma idêntica em `calcularPontosCopaGrupo` (CF `functions/index.js` e Dart `biblioteca.dart`) e no bônus inline da `tela_ranking._buildGrupoCopa`.
...
- **Detecção automática de fim da Fase de Grupos (`_faseGruposEncerrada`):** retorna `true` apenas quando o jogo 73 tem times reais (sem placeholder) **e** não há mais nenhum jogo de grupo (`group != null`) disponível para palpite em `_gruposProximos`. Com a API preenchendo os times dos 16-avos ainda durante a Rodada 3, checar só o jogo 73 causava sumiço dos jogos do dia — o critério duplo garante que a transição só ocorre após o cutoff do último jogo de grupos. Quando `true`: abas de modo somem e todos os jogos restantes aparecem em único Próximos/Encerrados
...
| `recalcularTudo` | HTTPS Callable (admin only) | Recalcula `pontuacaoClassica`, `pontuacaoEliminatorias`, `placaresExatos` e `palpitesPerdidos` do zero. Não toca em `pontuacaoEspeciais` nem `pontuacaoCopa`. Também reseta `copaGruposCalculado: false` em `config/copa2026`, liberando novo `recalcularCopa` (é o "rode a Clássica para resetar" do fluxo Copa). |
...
| `recalcularCopa` | HTTPS Callable (admin only) | Calcula pontos Copa fase de grupos (SET em `pontuacaoCopa`); marca `copaGruposCalculado: true`. Lê o conjunto de UIDs válidos de `usuarios` e **pula docs `palpites_copa` órfãos** (conta deletada) — `batch.update` em doc inexistente derrubaria todo o batch com NOT_FOUND (devolvido ao app como erro `INTERNAL`) |
| `limparUsuariosOrfaos` | HTTPS Callable (admin only) | Remove docs `usuarios` sem conta Auth + palpites órfãos (`palpites` **e `palpites_copa`**, doc ID = UID); tira UIDs órfãos dos arrays `membros` dos grupos (deleta grupos que ficarem vazios; transfere a posse se o dono era órfão). Retorno inclui `palpitesCopaRemovidos` |
...
- **Produção** — ✓ v1.0.0+23 promovida direto do Teste Interno para Produção (2026-08-02), sem re-upload do AAB. Versão anterior em produção era a v1.0.0+22 (2026-07-02).
- **targetSdk 36 / Android 16** — ✓ v1.0.0+23 (Flutter 3.44.8, `compileSdk`/`targetSdk` 36) gerada em 2026-08-02, atendendo à exigência do Google Play (prazo 31/ago/2026, alerta em "Status da política" no Play Console). Copa 2026 já encerrada e sem usuários ativos — build feito direto na `main`, sem branch separada. **Build passou a rodar via GitHub Actions** (`.github/workflows/build-release.yml`, disparo manual `workflow_dispatch`, `ubuntu-latest`): o build local nesta máquina (setup novo, apenas com o repo clonado do GitHub) ficou bloqueado por um bug de rede do Java no Windows (`Selector.open()` falha com `Unable to establish loopback connection` / socket AF_UNIX, reproduzido fora do Gradle e independente de JDK 20/21) — não é um problema do projeto. O workflow busca keystore, `key.properties`, `google-services.json` e `firebase_options.dart` de secrets do repositório (`KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_PASSWORD`, `KEY_ALIAS`, `GOOGLE_SERVICES_JSON_BASE64`, `FIREBASE_OPTIONS_DART_BASE64`) e publica o `.aab` como artifact. ✓ Testado no Teste Interno sem erros e **promovido para Produção com sucesso** (mesmo AAB, sem re-upload).
...
5. ~~Subir targetSdk para API 36 (Android 16) antes de 31/ago/2026.~~ ✓ v1.0.0+23 gerada via GitHub Actions, testada no Teste Interno sem erros e promovida para Produção (2026-08-02).
