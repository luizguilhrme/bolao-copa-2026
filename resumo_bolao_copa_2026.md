...
Usuários relataram que precisavam logar novamente toda vez que abriam o app instalado pela Play Store; em builds de debug o login persistia normalmente. Causa: o build de release usa `isMinifyEnabled = true`, e o R8 em **full mode** (padrão desde o AGP 8.0; o projeto usa 8.7.0) remove/renomeia código acessado por reflexão — exatamente o caminho que o Firebase Auth usa para **reler o JSON da sessão salva** no aparelho ao iniciar. A releitura falhava (`JSONArray[0] not a string` — [firebase-android-sdk#6375](https://github.com/firebase/firebase-android-sdk/issues/6375)), o SDK concluía que não havia usuário e `authStateChanges()` emitia `null` → tela de login. O comportamento variava por aparelho/sessão (reproduzido em Samsung; mesmo APK não falhava em outro tablet), o que dificultou o diagnóstico. **Corrigido com `android.enableR8.fullMode=false` em `android/gradle.properties`** — o R8 continua ligado (minificação + ofuscação + shrink de recursos), apenas no modo compatível, menos agressivo. Validado instalando o APK release no aparelho que reproduzia o bug: sessão passou a persistir. Nota: instalar um APK local por cima da versão da Play Store exige desinstalar antes (assinaturas diferentes — upload keystore × Play App Signing).
